<?php
    ...
    $select = "SELECT * FROM `producer` WHERE `Pd_id` = '$producer_update' ";
    $data_select = $Main->select_one($select);

    if (isset($_POST["btn_update_PD"])){
        $messenger = $Producer->update_producer();
        $error =$messenger[1];
    }
?>
            ...
            <div style="padding-left: 10px; border-left: 1px solid red;"" class=" col-xl-6">
                ...
                <div class="form-group">
                    ...
                    <textarea name="Adress" id="" cols="" rows="10"
                        class="form-control"><?php echo $data_select["Adress"]; ?></textarea>
                </div>
               
            </div>
        </div>
    </div>
    <button style="float: right; margin-right: 100px; margin-top: 30px;" class="btn btn-danger" name="btn_update_PD">Cập
        nhật</button>
        <br>
